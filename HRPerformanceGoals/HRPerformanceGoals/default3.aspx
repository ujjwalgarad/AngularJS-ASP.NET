<%@ Page Title="" Language="C#" MasterPageFile="~/Site1.Master" AutoEventWireup="true" CodeBehind="default3.aspx.cs" Inherits="HRPerformanceGoals.default3" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div class="container" ng-controller="MainCtrl" ng-cloak>
        <div ng-if="hasPermission">
            ...
            <div layout="row" style="background-color: #F5F5F5; border-radius: 8px; border: 1px solid #A9A9A9">
                <div flex="15">
                    <div style="padding: 10px">
                        <img class="imageContainer" id="idPersonImg" alt="Photo not available" ng-src="{{'images/1.JPG'}}">
            ...
            <div ng-if="isHR && StatusID==1">
                <%--ng-if="isHR && !releasedToManager"--%>
            ...
            <div ng-if="goals.length>0">
                ...
                <div layout="row"  style="padding-top:10px" ng-if="(isManager && StatusID>1) || isHR">
                    <md-card-title flex="{{StatusID==1?100:50}}">
                          <md-card-title-text>
                            <span class="md-headline">Goals :</span>
                          </md-card-title-text>
                    </md-card-title>
                    <md-card-title ng-if="StatusID>1" flex="50">
                          <md-card-title-text>
                    ...
                    <md-card ng-if="(isManager && StatusID>1) || isHR">
                        ...
                        <!----------------------------------------------------------------------Goal Section-------------------------------------------------------------------->
                            <div layout="row"> <%--flex="{{goal.Review || goal.reviewEdit ? 50 : 100}}"--%>
                                <div flex="{{StatusID==1?100:50}}" layout="row">
                                    <md-card flex="{{isHR && StatusID==1 ? 95:100}}">  <%--flex="{{isHR && !reviewCompleted ? 95:100}}--%>
                                        <md-card-content ng-if="!goal.editGoal">
                                            <label>Goal :</label>
                                            <div class="card-media" style="white-space: pre-wrap"><%--<i class="fa fa-flag-checkered fa-2x" aria-hidden="true"></i>&nbsp;&nbsp;--%><span>{{goal.Goal}}</span></div>
                                        </md-card-content>
                                        <md-card-content ng-if="goal.editGoal">
                                            <md-input-container class="md-block">
                                    ...
                                    <div layout="column"  flex="5" ng-if="isHR && StatusID==1"> <%--ng-if="!reviewCompleted && isHR"--%>
                                        <md-button title="{{ currentEditGoal!=-1 ? 'Please save/cancel the current editing goal.' : ''}}" ng-disabled="currentEditGoal!=-1" class="md-icon-button" aria-label="Edit Goal"  ng-if="!goal.editGoal" ng-click="editGoal(goal)">  <%--ng-if="!goal.editGoal && !releasedToManager"--%>
                                            <i ng-style="currentEditGoal!=-1 && {'color': '#dedede'}" class="icon-pencil"></i>
                                        </md-button>
                        ...
                        <div flex="50" layout="column" ng-if="(isManager && StatusID>1) || (isHR && StatusID>2)">
                                    <!----------------------------------------------------------------------30 Day Review Section-------------------------------------------------------------------->
                        <div layout="row"  ng-if="goal.reviewEdit || goal.Review">  <%--flex="50"--%>
                            <div flex="{{isManager && StatusID==2 ? 95:100}}" class="alert alert-warning" ng-if="(isManager && StatusID>1) || (isHR && StatusID>2)"> <%--flex="{{isManager && !reviewCompleted ? 95:100}}"--%>
                                <div ng-if="goal.Review && !goal.reviewEdit"> <%--style="background-color:#ECF5E0"--%>
                                        <label>30 Day Feedback :</label>
                                        <div class="card-media"  style="white-space: pre-wrap">{{goal.Review}}</div>
                                </div>
                                <div ng-if="goal.reviewEdit">
                                    <md-input-container class="md-block">
                            ...
                            <div><md-button class="md-raised" ng-disabled="currentEditReview!=-1" aria-label="Write Review" ng-if="!goal.Review && !goal.reviewEdit && StatusID==2 && isManager" ng-click="writeReview(goal)">Add 30-Day feedback</md-button></div>
                            <div><md-button class="md-raised pull-right" ng-disabled="currentEdit60DayReview!=-1" aria-label="Write Review" ng-if="!goal.Review60Day && !goal.review60DayEdit && StatusID==3 && isManager" ng-click="write60DayReview(goal)">Add 60-Day feedback</md-button></div>
                        ...
                        <div layout="row" ng-if="goal.review60DayEdit || goal.Review60Day">
                            ...
                            <%--<div layout="row" flex="50">--%>
                                <div flex="{{isManager && StatusID != 4 ? 95:100}}" class="alert alert-success" ng-if="(isManager && StatusID>2) || (isHR && StatusID>3)">
                                    <div ng-if="goal.Review60Day && !goal.review60DayEdit">
                                            <label>60 Day Feedback :</label>
                                            <div class="card-media"  style="white-space: pre-wrap">{{goal.Review60Day}}</div>
                                    </div>
                                    <div ng-if="goal.review60DayEdit">
                                        <md-input-container class="md-block">
